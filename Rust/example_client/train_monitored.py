...
to a monitoring server at http://localhost:8848
"""
...
def get_default_layer_groups(model_class):
    ...
    if model_class == DecentFashionCNN:
        return {
            "feature_extractor": ["conv1", "conv2", "conv3"],
            "classifier": ["fc1", "fc2"],
        }
    elif model_class == MinimalFashionCNN:
        return {"feature_extractor": ["conv1"], "classifier": ["fc"]}
    return None


def train_model(
    model_class=DecentFashionCNN,
    num_epochs=5,
    monitor_endpoint="http://localhost:8848/api/v1/metrics/layerwise",
    log_interval=10,
    batch_size=64,
    learning_rate=0.001,
    layer_groups=None,
    ...
    if device.type == "cuda":
        try:
            ...
            if major < 6:
                print(
                    f"GPU compute capability {major}.{minor} is too old. Falling back to CPU."
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--endpoint",
        type=str,
        default="http://localhost:8848/api/v1/metrics/layerwise",
        help="Monitor server API endpoint",
    )
    parser.add_argument(
        "--log-interval", type=int, default=10, help="Log metrics every N steps"
    )
